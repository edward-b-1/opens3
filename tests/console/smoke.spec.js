...
test('the menu button collapses the sections on wide screens and opens them on narrow ones', async ({ page }) => {
  const errors = watch(page);
  ...
  // Collapse, and the choice survives a reload.
  await toggle.click();
  ...
  await expect(page).toHaveURL(/#\/buckets/);
  // Narrow: the sections are a menu that closes on choice.
