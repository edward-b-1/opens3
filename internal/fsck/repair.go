...
// within returns the absolute path of rel under root, or an error when it
// escapes root (a report is generated by Check, but the repair must not
// trust it with the operator's filesystem).
func (r *repairer) within(rel string) (string, error) {
	root, err := filepath.Abs(r.root)
	if err != nil {
		return "", err
	}
	if filepath.IsAbs(rel) || strings.HasPrefix(rel, "/") {
		return "", fmt.Errorf("path %q is outside the data directory", rel)
	}
...
// bucketDir is the data directory of a bucket, refusing names that are not
// bucket names.
func (r *repairer) bucketDir(bucket string) (string, error) {
	if bucket == "" || strings.ContainsAny(bucket, "/\\") || bucket == "." || bucket == ".." {
		return "", fmt.Errorf("%q is not a bucket name", bucket)
	}
	return r.within(filepath.Join("data", bucket))
}
...
func (r *repairer) removeParts(p Problem) error {
	...
	for _, b := range blobs {
		...
		if strings.ContainsAny(b, "/\\") || b == ".." {
			return fmt.Errorf("%q is not a blob id", b)
		}
		path := filepath.Join(dir, b[:2], b)
		if err := os.Remove(path); err != nil && !errors.Is(err, os.ErrNotExist) {
			return err
		}
	}
